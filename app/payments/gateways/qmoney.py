"""
QMoney Payment Gateway Integration
"""
...
class QMoneyGateway(BasePaymentGateway):
    """QMoney payment gateway implementation."""
    ...
    def initiate_payment(self, amount: float, reference: str, order_id: int,
        ...
        if 'phone' not in customer_info:
            raise PaymentGatewayException("Phone number is required for QMoney payments")
        ...
        try:
            ...
            raise PaymentGatewayException(f"Failed to initiate QMoney payment: {str(e)}")
    
    def verify_payment(self, reference: str, transaction_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Verify QMoney payment status.
        ...
        try:
            ...
            raise PaymentGatewayException(f"Failed to verify QMoney payment: {str(e)}")
    
    def process_webhook(self, payload: Dict[str, Any], signature: Optional[str] = None) -> Dict[str, Any]:
        """
        Process QMoney webhook notification.
